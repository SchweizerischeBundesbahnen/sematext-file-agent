...
var Logagent = require('./agent/lib/parser/parser');
var fs = require('fs')
var loggers = {}
...
function getLogger(token, podName) {
    var key = token + podName;

    if (!loggers[key]) {
        var logger = new Filelogger(token, podName)
...
function logToLogsene(level, message, data) {
    ...
    if (dockerInspect) {
        var logger = getLogger(dockerInspect.SPLUNK_INDEX, data.kubernetes.pod_name)
        ...
        if (dockerInspect.Config && dockerInspect.Config.Labels &&
            dockerInspect.Config.Labels['com.docker.compose.project']) {
        ...
        getLogseneTokenByContainer(data.container_id, function (err, info) {
            var token = process.env.SPLUNK_INDEX
            if (!err && info) {
                dockerInspectCache[data.container_id] = info
                if (info.SPLUNK_INDEX) {
                    token = info.SPLUNK_INDEX
                }
            }
            if (token) {
                var logger = getLogger(token, data.kubernetes.pod_name)
                logger.log(level, message, data)
            }
        })
    }
...
function createLogAgent() {
    ...
    if (fs.existsSync(patternFile)) {
        console.log(new Date().toISOString() + ' - INFO - Loading custom log parser definitions: ' +
                    patternFile)
        return new Logagent(patternFile)
    } else {
        console.log(new Date().toISOString() + ' - INFO - Use -v /mypattern/patterns.yml:' + patternFile +
                    ' for custom log parser definitions.')
...
function DockerLogsene() {
    ...
    this.opts = {
        ...
        docker: null,
        events: null,
        includeCurrentContainer: false, // the following options limit the containers being matched
    ...
    if (process.env.ENABLE_LOGSENE_STATS === 'true') {
        console.log('logging of stats is enabled');
        setInterval(function () {
            var logInfoString = 'SDA log stats: count=' + self.logCount + ' shipped=' + logsShipped +
                                ' bulks_req_send=' + httpSend + ' bulk_req_failed=' + httpFailed +
                                ' bulk_req_retransmit=' + httpRetransmit
            if (process.env.SPM_LOG_TO_CONSOLE) {
                console.log(logInfoString)
            }
            var logger = getLogger(process.env.SPLUNK_INDEX, 'sda-logging-stats')
...
DockerLogsene.prototype.getLogObject = function (line, container, cbf) {
    if (this.imageNameRegEx.test(container)) {
...
DockerLogsene.prototype.enrichAndLog = function (err, logObject) {
    if (err && !logObject) {
        ...
        if ((Date.now() - logObject['@timestamp'].getTime()) < (90 * 24 * 60 * 60 * 1000)) {
            logToLogsene(String((logObject.level || logObject.lvl || logObject.severity || 'info')),
...
var nonPrintable = / [^\x20-\x7E]/g
DockerLogsene.prototype.logLine = function (messageText, data, next) {
    var self = this
    var dockerInspect = dockerInspectCache[data.id]
    if (dockerInspect && dockerInspect.LOGSENE_ENABLED === true) {

        if (messageText.includes('\x00') || messageText.includes('x00')) {
            console.log('now', messageText);
        }

        // Remove non-printable characters
        messageText.replace(nonPrintable, '');

        var logContext = {
            parseKubernetesInfo: self.parseKubernetesInfo,
            sourceName: data.image + '_' + data.name + '_' + data.id,
            chunk: data,
            line: (process.env.REMOVE_ANSI_ESCAPE_SEQ === 'enabled') ?
