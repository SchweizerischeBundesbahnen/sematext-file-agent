...
var Logagent = require('./lgagent/lib/parser/parser');
var fs = require('fs')
var loggers = {}
...
function logToLogsene(level, message, data) {
    ...
    if (dockerInspect) {
        var logger = getLogger(dockerInspect.SPLUNK_INDEX, message.kubernetes.pod_name)
        ...
        getLogseneTokenByContainer(data.container_id, function (err, info) {
            ...
            if (token) {
                var logger = getLogger(token, message.kubernetes.pod_name)
                logger.log(level, message, data)
            }
        })
    }
