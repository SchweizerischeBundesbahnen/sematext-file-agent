...
function getLogger(token) {
    var key = token
    if (!loggers[key]) {
        var logger = new Filelogger(token)
...
function logToLogsene(level, message, data) {
    ...
    if (dockerInspect) {
        var logger = getLogger(dockerInspect.SPLUNK_INDEX)
...
DockerLogsene.prototype.getLogObject = function (line, container, cbf) {
    ...
    if (line && line.length > 0) {
        console.log("new line. calling parser with: ", line);
        this.logagent.parseLine(line, container, function (err, parsed) {
            if (/log-test/.test(container)) {
...
DockerLogsene.prototype.enrichAndLog = function (err, logObject) {
    console.log('enrichAndLog', logObject.message);
    if (err && !logObject) {
        return
    } else {
        ...
        if ((Date.now() - logObject['@timestamp'].getTime()) < (90 * 24 * 60 * 60 * 1000)) {
            console.log('sending to logsense', logObject.message);
            ...
            console.log('append to existing logObject');
        }
    }
}
...
DockerLogsene.prototype.logLine = function (messageText, data, next) {
    ...
    var logContext = {
        ...
        chunk: data,
        line: (process.env.REMOVE_ANSI_ESCAPE_SEQ === 'enabled') ? messageText.replace(ansiEscapeRegEx, '') :
              messageText
    }
    console.log('new logContext now: ', logContext.line);
    setImmediate(function () {
        console.log('> self.getLogObject', logContext.message);
