...
var logPath = process.env.LOGFILE_PATH || '/var/log/splunk-';
...
fileLogger.prototype.log = function (level, message, fields, callback) {
    ...
    if (msg.message && Buffer.byteLength(msg.message, 'utf8') > MAX_MESSAGE_FIELD_SIZE) {
        ...
        msg.logsene_client_warning =
        'Warning: message field too large > ' + MAX_MESSAGE_FIELD_SIZE + ' bytes'
    }
    // Own log line
    var logRowFormatted = `INDEX: ${_index} | POD_NAME: ${msg.kubernetes.pod_name} | NAMESPACE: ${msg.kubernetes.namespace} | HOST: ${msg.host} | SEVERITY: ${msg.severity} | MESSAGE: ${msg.message}`;
    winston.log('info', logRowFormatted);
