...
MultiLine.prototype.lineTimeout = function () {
    if (this.lines.length > 0) {
        ...
        if (this.sourceName.includes('novaan') && !l.startsWith("2017")) {
            console.log( 'lineTimeout Reached', l.substr(0, 50), '||', l.substr(l.length - 50, 50), '||', this.sourceName);
        }
...
MultiLine.prototype.add = function (line, cbf) {
    ...
    if (this.lines.length === 0) {
        if (this.sourceName.includes("novaan")) {
            console.log('adding first line', line.substr(0, 50));
        }
