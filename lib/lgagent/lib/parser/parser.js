            ...
            if (!err && cityLookup) {
                ...
                if (this.cfg.globalTransform) {
                    this.cfg.globalTransform = this.cfg.globalTransform.bind({
                        moment: moment, geoip: geoipCity, enrichGeoIp: this.enrichGeoIp
                    })
                }
            }
            if (this.readyCallback) {
...
LogParser.prototype = {
    initMaxmind: function () {
        ...
        consoleLogger.log('hot reload pattern files: ' + changedFile + ' modified -> reload all ' +
                          filesToLoad)
        ...
        if (geoipCity != null && parsedObject[fieldName]) {
            ...
            if (location && location.country && location.city && location.city.names &&
                location.city.names.en) {
                parsedObject['geoip'] = {
                    location: [location.location.longitude, location.location.latitude], info: {
        ...
        if (src && src.reader) {
            ...
            if (include.length > 0) {
                console.log('new multiline parser', sourceName, include[0].blockStart);
                this.sources[sourceName].reader = new MultiLine(include[0].blockStart, parser, sourceName.includes('novaan'));
                return this.sources[sourceName].reader
            } else {
    ...
    bubbleUp: function (plist, pos) {
        ...
        function mlReaderCb(data) {
            setImmediate(function parseLineWrapper() {
                call(self._parseLine.bind(self, data, source, cbf), parseResultHandler.bind({cbf: cbf}))
            })
        }
        ...
        if (this.cfg.globalTransform) {
            call(this.cfg.globalTransform.bind(null, source, parsed), function (err, result) {
                if (err) {
        ...
        if (this.patternMatchingDisabled || this.sourceBlacklist[source] >= this.MAX_TRAINING_LINES) {
            console.log('inside blacklist', this.sourceBlacklist[source], this.MAX_TRAINING_LINES);
            ...
            for (var k in patternList) {
                ...
                for (var i = 0; i < patterns.match.length; i++) {
                    if (this.matchPatterns(patterns.match[i], parsed, line)) {
                        ...
                        if (source.type === 'sbb') {
                            console.log('line did not match pattern: ', line, patterns.match[i]);
                        }
                    }
                }
            }
        } else {
            console.log('pattern not found. going to blacklist.', source, line);
        }

        this.sourceBlacklist[source] = (this.sourceBlacklist[source] || 0) + 1
        if (source.includes('novaan')) {
            console.log('adding to blacklist', line, source);
        }
